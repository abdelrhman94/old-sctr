...
import { SaveSponsorsRequest } from "@/models/Studies.model";
import { useLazyGetSponsorsQuery } from "@/store/api/studiesApi";
...
const ReviewSponsors = ({ navigationDirection, studyId, setNavigationDirection }: Props) => {
  const dispatch = useDispatch()
  const [summaryData, setSummaryData] = useState<SaveSponsorsRequest>();
            ...
            <div className="flex-grow space-y-8 my-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                <BaseInput
                  ...
                  type="text"
                  value={summaryData?.isThereAnySponsors ? "Yes" : 'No Sponsors'}
                />
              </div>
              {summaryData?.isThereAnySponsors && (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                    <BaseInput
                      ...
                      label="Address"
                      type="text"
                      value={summaryData?.sponsorCity}
                    />
                    <BaseInput
                      disabled
                      label="Contact Person"
                      type="text"
                      value={summaryData?.sponsorPhone}
                    />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                    <BaseInput
