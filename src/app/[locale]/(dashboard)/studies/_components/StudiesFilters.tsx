...
export default function StudiesFilters({
  date, studyType, studyStatus, setQuery, onChanged,
}: {
  date: Date | null;
  ...
  const { data: studyTypesLookup, isLoading: studyTypesLoading } = useGetLookupByNameQuery({ name: 'EnumStudyType' });
    ...
    <div className="rounded-xl border bg-white p-3 flex items-center gap-4">
      ...
      <div className="border-l pl-4">
        <Select
          ...
          onValueChange={(val) => {
            setQuery({ studyType: val === "-1" ? null : Number(val), page: 1 });
            onChanged?.();
          }}
        >
          ...
          <SelectContent>
            ...
            {!studyTypesLoading &&
              (studyTypesLookup?.data?.EnumStudyType?.length ? (
              studyTypesLookup.data.EnumStudyType.map((item: Lookups) => (
      ...
      <div className="border-l pl-4">
        <Select
          value={studyStatus?.toString() ?? ""}
          onValueChange={(val) => {
            setQuery({ studyStatus: val === "-1" ? null : Number(val), page: 1 });
            onChanged?.();
          }}
        >
          <SelectTrigger className="w-40">
            <SelectValue placeholder="Study Status" />
          </SelectTrigger>
          <SelectContent>
            {/* "All" sentinel */}
      ...
      <div className="ml-auto border-l pl-4">
        <Button
          ...
          onClick={() => {
            setQuery({ date: null, studyType: null, studyStatus: null, search: "", page: 1 });
          }}
        >
